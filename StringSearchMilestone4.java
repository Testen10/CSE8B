...
class StringSearchMilestone4{
    ...
    public static void main(String[] args){
        ...
        boolean q_not = false;
        ArrayList<Query> qcommand_list = new ArrayList<Query>();
        ArrayList<Transform> tcommand_list = new ArrayList<Transform>();
        ...
        if(args.length > 1){ 
            ...
            for(int loc = 0;loc<queryInput_arr.length;loc++){
                qcommand_list.add(readQuery(queryInput_arr[loc]));
            }
        ...
        if(args.length > 2){ 
            ...
            for(int loc = 0;loc<transformInput_arr.length;loc++){ 
                tcommand_list.add(readTransform(transformInput_arr[loc]));
            }
        }
        ...
        for(String elem:fileHelper.getLines(args[0])){
            String output = elem;
            for(Query query:qcommand_list){
                if(query==null){return;}
                if(!query.matches(elem)){
                    output = "";
                    break;
                }
            }
            if(!output.equals("")){
                for(Transform transform:tcommand_list){
